...
class HyperFramesCompose(BaseTool):
    ...
    def _scaffold(self, inputs: dict[str, Any]) -> ToolResult:
        """Materialize the HyperFrames workspace from Montara artifacts.
        ...
        html = self._generate_index_html(
            ...
            or f"Montara {edit_decisions.get('renderer_family', 'composition')}",
        )
    ...
    def _style_bridge(
        ...
        """Bridge Montara playbook → HyperFrames CSS vars + DESIGN.md.
        ...
        design_md = (
            "# DESIGN\n\n"
            "Generated by Montara HyperFrames style bridge (fallback).\n\n"
            f"- Background: `{bg}`\n"
            f"- Foreground: `{fg}`\n"
            f"- Accent: `{accent}`\n"
            f"- Primary: `{primary}`\n"
