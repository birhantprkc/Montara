...
Montara pull requests. It is a review framework, not a closed checklist.
Use it to structure the review, then keep looking for issues that are specific
to the PR in front of you.

Montara is an agent-orchestrated video production system. Many PRs affect
...
- Does this PR move Montara in the right direction?
...
Check whether the PR solves a real Montara problem.
...
Montara works with video, audio, image generation, and local models. Small
...
## Montara Architecture Checks

### Agent-First Architecture

Montara's control plane is the agent following markdown skills and YAML
...
Montara uses checkpoints for resume, human approval, audit trails, and
stage gating.
...
Montara can compose with Remotion, HyperFrames, or FFmpeg. Runtime choices
...
Montara should not silently spend money or imply a paid provider is free.
...
- Is this provider useful for Montara workflows?
...
- Is the provider internationally known, nationally or regionally important,
  or otherwise clearly valuable to Montara users?
...
Provider viability matters. Montara should not become a grab bag of
