...
This is the **Montara-specific** guide to HyperFrames. It explains when
Montara pipelines should choose HyperFrames over Remotion, how Montara
...
## When Montara should pick HyperFrames (vs Remotion vs FFmpeg)

Montara separates two concepts:
...
| Synthetic UI / fake terminal / fake browser demo | Either — depends on existing coverage | Montara already ships Remotion `TerminalScene` (see `synthetic-screen-recording` Layer 3). For UI chrome beyond terminal, HyperFrames HTML is easier. |
...
Montara artifacts into HyperFrames project files:

| Montara artifact field | HyperFrames target |
|---|---|
...
`STORYBOARD.md` as step-by-step workspace files. Montara does **not**
...
Montara playbooks currently translate into Remotion `themeConfig`
...
- ❌ Running `hyperframes init` from the Montara orchestrator. `init`
