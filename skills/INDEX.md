...
Layer 2: skills/                          "How Montara uses these tools"
...
3. Layer 2 skills (this directory) teach the agent Montara-specific conventions
...
| **Image Generation** | `flux-best-practices`, `bfl-api`, `grok-media` | `black-forest-labs/skills`, local Montara skill |
...
| **AI Video (Premium)** | `seedance-2-0` — preferred premium default (cinematic, trailer, multi-shot, lip-sync, synced audio); accessed via `seedance_video` (fal.ai) or `heygen_video` Avatar Shots | Local Montara skill |
